(function(document, $, Coral) {
    ...
    var staticTextRTE = ".coral-RichText";
    ...
    function updateLinkText(target) {
        ...
        if (url && url.startsWith("/")) {
            var rte = target.parents("coral-multifield-item").find(staticTextRTE);
            var rtetext =target.parents("coral-multifield-item").find(".cq-RichText :input"); 
            if (rte) {
                $.ajax({
                    url: url + "/_jcr_content.json"
                }).done(function(data) {
                    if (data) {
                        rte.html(data["jcr:title"]);
                        rtetext.val(data["jcr:title"]);
                    }
                });
            }
        }
